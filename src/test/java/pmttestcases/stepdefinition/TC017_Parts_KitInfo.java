...
public class TC017_Parts_KitInfo extends Baseclass{
	...
	public void user_will_go_to_parts_page_and_search_partno() throws Throwable {
		try {
			//extent=setup();	
			test = extent.createTest(Feature.class, "PMT parts kit info add, edit and delete").assignCategory("Parts KitInfo").pass("KitInfo add");
	...
	public void user_will_select_des_code_Language_and_Enter_maintanence_type_Qty() throws Throwable {
		try {
			loginfo=test.createNode(new GherkinKeyword("And"),"User will select des code & Language and Enter maintanence type & Qty");
			pkinfo.FillKitDetails();
			loginfo.pass("Des code & Language selected and maintanence type & Qty Entered Succesfully");
			loginfo.addScreenCaptureFromPath(Screenshotcapture(driver));
			
		} catch(Exception e) {
			System.out.println("Des code & Language Not selected or maintanence type & Qty Not Entered" + e.getMessage());
	...
	public void use_will_Select_Sold_Separately_Qty_Uom_and_Enter_Description() throws Throwable {
		try {
			loginfo=test.createNode(new GherkinKeyword("Then"),"Use will Select Sold Separately & Qty Uom and Enter Description");
	...
	public void user_will_verify_reocrd_has_shown_in_the_below_grid() throws Throwable {
		try {
			loginfo=test.createNode(new GherkinKeyword("Then"),"User will verify reocrd has shown in the below grid");
			pkinfo.Verifykit();
	...
	public void user_will_click_parts_page_and_search_partno() throws Throwable {
		try {
			//extent=setup();	
			test = extent.createTest(Feature.class, "PMT parts kit info add, edit and delete").assignCategory("Parts KitInfo").pass("KitInfo Edit");
	...
	public void user_will_click_parts_page_and_search_for_partno() throws Throwable {
		try {
			//extent=setup();	
			test = extent.createTest(Feature.class, "PMT parts kit info add, edit and delete").assignCategory("Parts KitInfo").pass("KitInfo Delete");
	...
	public void user_will_Accept_the_alert_and_Verify_that_record_not_shown_in_the_below_grid() throws Throwable {
		try {
			loginfo=test.createNode(new GherkinKeyword("And"),"User will Verify that record not shown in the below grid");
			pkinfo.VryDelete();
			loginfo.pass("Verified Succesfully");
			loginfo.addScreenCaptureFromPath(Screenshotcapture(driver));
			//extent.flush();
